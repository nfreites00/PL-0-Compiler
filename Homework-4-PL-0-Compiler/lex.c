...
lexeme *lex_analyze(char *inputFile, int lFlag) {
  // An array that holds types for the lexeme list.
	...
	// String array that holds the reserved words. If a string matches any of the words when being compared, it is a reserved word.
 	const char reserved_words[15][11] = {"const", "var", "procedure", "call", "begin", "end", "if", "then", "else", "while", "do", "read", "write", "odd", "null"};
	...
	while(inputFile[charArr_index] != '\0') {
		...
		// Checks if current char is an letter.
            ...
            // procedure
            case 2:
              lexemeArray[ll_index].type = procsym;
							ll_index++;
              if(lFlag == 1) {
                printf("procedure\t\t%d\n", procsym);
              }
            ...
            // call
            case 3:
              lexemeArray[ll_index].type = callsym;
							ll_index++;
              if(lFlag == 1) {
                printf("call\t\t\t%d\n", callsym);
              }
              ...
              if(lFlag == 1) {
                printf("then\t\t\t%d\n", thensym);
							...
							break;
            case 8:
              lexemeArray[ll_index].type = elsesym;
							ll_index++;
              if(lFlag == 1) {
                printf("else\t\t\t%d\n", elsesym);
              }
